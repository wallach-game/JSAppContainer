/**
 * Generates the HTML content for the JSCAD rendering iframe.
 * Uses CDN-loaded @jscad/modeling and Three.js for mobile-compatible 3D rendering.
 */
export function buildRendererHtml(jscadCode: string, height: number): string {
	const escapedCode = jscadCode
		.replace(/&/g, "&amp;")
		.replace(/</g, "&lt;")
		.replace(/>/g, "&gt;")
		.replace(/"/g, "&quot;");
